...
func set_active_tool(tool: String) -> void:
	active_tool = tool
	...
	match active_tool:
		...
		"water":
			if soilMode >= SOIL_MODE_NAMES.plant0 and soilMode <= SOIL_MODE_NAMES.plant2:
				_water()
		"seeds":
			if soilMode == SOIL_MODE_NAMES.soilplain:
				_plant()
			pass
		_:
			pass
...
func _till() -> void:
	soilMode = SOIL_MODE_NAMES.plant0
	health = 0
	_update_sprite()
	timer.start()
	emit_signal("plant", soil_id)
...
func _harvest() -> void:
	...
	_update_sprite()
	timer.stop()
	emit_signal("harvest", soil_id)

func _water() -> void:
	health += 1
	emit_signal("water", soil_id)
